...
import DatePicker from './MobileDatePickers.tsx';
import DatePickertwo from './MobileDatePickerstwo.tsx';
  ...
  return (
    <div className="relative">
      <button 
        ...
        className={`
          w-full md:w-[120px] h-8 px-2 text-xs border border-[#D2D2D2] rounded-[10px] 
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <svg width="20" height="20" viewBox="0 0 20 20" className={`transform transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}>
            <path d="M5 8L10 13L15 8" stroke="#364A15" strokeWidth="1.5" fill="none"/>
          </svg>
        </div>
      </button>

      {isOpen && (
        <div className="absolute w-full md:w-[120px] bg-white border border-t-0 border-[#D2D2D2] rounded-b-[10px] z-50">
          <div className="px-2">
          ...
          {options.map((option) => (
            <div
              ...
              className={`px-2 py-1.5 text-xs rounded-xl cursor-pointer transition-colors duration-150 ${value === option ? 'bg-gray-50' : ''}`}
            >
              {option}
            </div>
          ))}
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <div className={`inline-flex items-center gap-1 px-2 h-6 rounded-full text-white text-xs ${isActive ? 'bg-[#1AC84B]' : 'bg-gray-400/60'}`}>
      {status}
...
const CampaignItems = () => {
  ...
  return (
    <div className="relative px-4 md:px-0">
      <div className="flex justify-between items-center mb-4 w-full md:w-[843px]">
        <h2 className="text-lg font-medium tracking-wide text-[#364A15]">Campaign</h2>
        <button 
          onClick={() => setShowModal(true)}
          className="inline-flex items-center gap-2 px-4 md:px-6 py-2 bg-[#D2F4D6] rounded-full text-sm text-[#364A15]"
        >
          Add
        </button>
      </div>

      <div className="w-full md:max-w-[843px] overflow-x-auto">
        <div className="min-w-[843px]">
          <div className="bg-white rounded-[26px] px-4 md:px-[50px] py-3 mb-2.5">
            ...
            <div className="flex flex-col gap-2.5">
              {campaigns.map((campaign, index) => (
                <div
                  ...
                  className="bg-white rounded-[26px] px-4 md:px-[50px] py-3.5 flex items-center min-h-[54px] cursor-pointer hover:bg-gray-50 transition-colors"
                >
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] p-4">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl w-full max-w-[800px] max-h-[90vh] overflow-y-auto p-4 md:p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-x-8 md:gap-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full md:w-[200px] h-8 px-2 text-xs border border-[#D2D2D2] rounded-[10px] focus:outline-none focus:ring-2 focus:ring-[#D2F4D6]"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder="10"
                  className="w-full md:w-[120px] h-8 px-2 text-xs border border-[#D2D2D2] rounded-[10px] focus:outline-none focus:ring-2 focus:ring-[#D2F4D6]"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder="50000"
                  className="w-full md:w-[230px] h-8 px-2 text-xs border border-[#D2D2D2] rounded-[10px] focus:outline-none focus:ring-2 focus:ring-[#D2F4D6]"
                />
              ...
              <div className="col-span-1 md:col-span-2">
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  className="w-full md:w-[350px] h-20 p-2 text-xs border border-[#D2D2D2] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D2F4D6] resize-none"
                />
              </div>
            </div>
